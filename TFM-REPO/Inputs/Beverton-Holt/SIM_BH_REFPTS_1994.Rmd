---
title: "Ejercicio Stock-Recruitment Reference Points 1994 BH CV02"
...
output_dir <- "C:/Users/Tito/Documents/Tito CDS/MTE/TFM IEO/Case Studies/Imagenes/BHsim_1994"

# Combinamos la carpeta con el nombre del archivo
output_file <- file.path(output_dir, paste0(params$n_iter,"_",params$cv,"_",params$model_type,"_1994_plot_series.png"))
...
# Eliminamos los de indices negativos
indbneg<-which(b<=0)
indaneg<-which(a<=0)
indneg<-unique(c(indaneg, indbneg))
numiter<-params$n_iter-length(indbneg)
# numiter
data<-data.frame(SSB=0, REC=0,ITER=0)
vec11<-1:params$n_iter
if(length(indneg)>0){vec11=vec11[-indneg]}
  for (i in vec11) {
    if (params$model_type == "BH") {
...
A continuación evaluamos donde suceden los valores no deseados y en que iteración se generan ya que se observó que se generaban valores negativos al ajustar mediante BH con CV de 0,6 y superiores.

```{r Evaluación de valores no deseados}

paste0("Los números de las iteraciones en las que se generan valores no deseados son: ",list(vec11))

# Se comprueba que el valor inicial introducido para params$n_iter genera el número deseado de valores adecuados.

paste0("El número de valores adecuados es de: ",length(vec11), " en ",params$n_iter, " iteraciones.")
...
# Combinamos la carpeta con el nombre del archivo
output_file <- file.path(output_dir, paste0(params$n_iter,"_",params$cv,"_",params$model_type,"_1994_plot_SR.png"))
...
# Combinamos la carpeta con el nombre del archivo
output_file <- file.path(output_dir, paste0(params$n_iter,"_",params$cv,"_",params$model_type,"_1994_Boxplot_parametros.png"))
...
```{r Puntos de referencia Setup y eliminación de NA y valores negativos}
system.time(
...
# Eliminamos los de indneg
ssb_virgin<-ssb_virgin[-indneg]
ssb_msy<-ssb_msy[-indneg]
f_msy <- f_msy[-indneg]
f_msy_yield <- f_msy_yield[-indneg]
Fcrash<- Fcrash[-indneg]
```
...
```{r gráfico boxplot OUTDATED,include=FALSE,eval=FALSE}
# refp1<-ggplot(data = df_long_ssb, aes(x = RPs, y = SSB)) +
#   geom_boxplot() +
#   labs(title = "Reference Points") +
#   theme_minimal()

# refp2<-ggplot(data = df_long_f, aes(x = RPs, y = F)) +
#   geom_boxplot() +
#   theme_minimal()

# refp3<-ggplot(data = df_long_yield, aes(x = RPs, y = Yield)) +
#   geom_boxplot() +
#   theme_minimal()
# 
# refp_grid<-(refp1) + (refp2) + (refp3)
# plot(refp_grid)
```
...
plot_combinado <- (p1 | p2 | p3 | p4 | p5) + 
  plot_annotation(title = "Boxplots: Puntos de Referencia (Overview)")
...
# Combinamos la carpeta con el nombre del archivo
output_file <- file.path(output_dir,paste0(params$n_iter,"_",params$cv,"_",params$model_type, "_1994_Boxplot_RPs.png"))
...
A continuación se muestra en el siguiente bloque de código la función empleada para el cálculo y la evaluación de la convergencia, de modo que podamos calibrar el número de iteraciones requerido para obtener resultados satisfactorios.
...
# Combinamos la carpeta con el nombre del archivo
 output_file <- file.path(output_dir,paste0(params$n_iter,"_",params$cv,"_",params$model_type,"_1994_SSBmsy.png"))
...
# Combinamos la carpeta con el nombre del archivo
 output_file <- file.path(output_dir,paste0(params$n_iter,"_",params$cv,"_",params$model_type,"_1994_Fmsy.png"))
...
# Combinamos la carpeta con el nombre del archivo
 output_file <- file.path(output_dir,paste0(params$n_iter,"_",params$cv,"_",params$model_type,"_1994_Fcrash.png"))
...
# Combinamos la carpeta con el nombre del archivo
 output_file <- file.path(output_dir,paste0(params$n_iter,"_",params$cv,"_",params$model_type,"_1994_SSBvirgin.png"))
...
# Combinamos la carpeta con el nombre del archivo
 output_file <- file.path(output_dir,paste0(params$n_iter,"_",params$cv,"_",params$model_type,"_1994_MSY.png"))
...
paste0(params$n_iter,"_",params$cv,"_",params$model_type,"_1994_", names(list_cvs)," = ",list_cvs) 
```
...
save(lista_params,file = paste0(params$n_iter,"_",params$cv,"_",params$model_type," Variables_BH_1994.RData"))
```